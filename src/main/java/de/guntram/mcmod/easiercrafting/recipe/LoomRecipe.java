...
public record LoomRecipe(
     ...
     */
    public static Optional<LoomRecipe> fromItemStack(ItemStack stack, String ip) {
        var component = stack.get(DataComponents.BANNER_PATTERNS);
        ...
        for (var layer : component.layers()) {
            ...
            steps.add(new BannerStep(layer.color().name(), layer.pattern().getRegisteredName()));
        }

        return Optional.of(new LoomRecipe(
                null,
                ip,
                stack.getItem().getDescriptionId(), //net.minecraft.registry.Registries.ITEM.getId(stack.getItem()).toString(),
                steps
        ));
    }
}